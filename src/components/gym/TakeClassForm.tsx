  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      if (existingEnrollment) {
        toast.error("Member already registered for this class");
        return;
      }
      ...
      toast.success("Member enrolled in class successfully");
      form.reset();
    } catch (error: any) {
                ...
                <SelectContent>
                  {classes?.map((cls) => (
                    <SelectItem key={cls.class_id} value={cls.class_id.toString()}>
                      {cls.class_name} - {cls.schedule_time} (Capacity: {cls.max_capacity})
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
